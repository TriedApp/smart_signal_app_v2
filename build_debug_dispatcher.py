...
def run_strategy(df, symbol):
    print(f"🔍 بررسی نماد: {symbol} → تعداد کندل‌ها: {len(df)}")
    ha = heikin_ashi(df)
    ...
    for i in range(1, len(df)):
        ...
        print(f"🔎 i={i} | bb={row['bb_percent_b']:.2f} | macd={row['macd']:.2f} | stoch={row['stoch_rsi']:.2f} | engulf={'BULL' if is_bullish_engulfing(prev,row) else 'BEAR' if is_bearish_engulfing(prev,row) else 'NONE'} | ha={'UP' if row['ha_close'] > row['ha_open'] else 'DOWN'}")
...
def fetch_ohlcv(symbol, interval="15m", limit=300):
...
def main():
    symbols = ["BTCUSDT", "ETHUSDT", "XRPUSDT", "LTCUSDT", "DOGEUSDT"]
    for symbol in symbols:
        df = fetch_ohlcv(symbol)
        if df is None or df.empty:
            print(f"⚠️ داده‌ای برای {symbol} دریافت نشد.")
            continue
        signals = run_strategy(df, symbol)
        if signals:
            for s in signals:
                print(f"✅ سیگنال صادر شد: {s}")
        else:
...
print(f"✅ نسخه دیباگ‌شده dispatcher با لاگ کامل ساخته شد و جایگزین شد:\n{target_path}")