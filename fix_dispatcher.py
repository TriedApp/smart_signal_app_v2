...
def heikin(df):
    ...
    if df.empty or len(df) < 2:
        ...
        h['ha_low'] = np.nan
        return h
    h['ha_close'] = (df['open'] + df['high'] + df['low'] + df['close']) / 4
    ho = [(df['open'].iloc[0] + df['close'].iloc[0]) / 2]
    for i in range(1, len(df)):
        ho.append((ho[i - 1] + h['ha_close'].iloc[i - 1]) / 2)
    h['ha_open'] = ho
    h['ha_high'] = df[['high', 'low', 'open', 'close']].max(axis=1)
    h['ha_low'] = df[['high', 'low', 'open', 'close']].min(axis=1)
    return h
...
print("✅ فایل signal_dispatcher.py با موفقیت اصلاح شد و آماده‌ی اجراست.")