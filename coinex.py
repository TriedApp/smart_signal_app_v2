...
def fetch_kline(symbol, interval):
    url = f"https://api.coinex.com/v2/market/kline?market={symbol}&interval={interval}&limit=100"
    try:
        res = requests.get(url, timeout=10)
        raw = res.json()
        if raw.get("code") != 0 or "data" not in raw:
            print(f"⚠️ پاسخ نامعتبر برای {symbol} | تایم‌فریم: {interval}")
            return None
        data = raw["data"]
